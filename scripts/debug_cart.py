...
Usage: Run this script to verify that products can be added to the cart correctly.
       This includes testing sold-out detection and cart drawer verification.

When to use:
- Testing cart.py after modifications
- Debugging "Add to Cart" button detection issues
- Verifying cart drawer appears correctly
- Testing sold-out product handling

Requirements:
- .env.local with valid credentials
- HEADLESS=false recommended for visual debugging
- TEST_PRODUCT_URL should point to an in-stock product

Note: This script uses a test product (Blackwell Jamaican rum) that is typically
      in stock. Update TEST_PRODUCT_URL if needed.
"""
